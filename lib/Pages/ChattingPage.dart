...
class ChatScreenState extends State<ChatScreen> {
  ...
  createListMessages(){
    return Flexible(
      child: chatId==""? Center(
        ...
        stream: Firestore.instance.collection("message").document(chatId).collection(chatId).orderBy('timestamp',descending: true).limit(20).snapshots(),
          ...
          Row(
            children: <Widget>[
              ...
              //text
                ...
                padding: EdgeInsets.fromLTRB(15.0, 15.0, 15.0, 15.0),
                width: 200.0,
                decoration: BoxDecoration(color: Colors.lightGreenAccent,borderRadius: BorderRadius.circular(8.0)),
                margin: EdgeInsets.only(left: 10.0),
              )
              :
              document["type"]==1?Container(
                child: FlatButton(
                  child: Material(
                    child: CachedNetworkImage(
                      placeholder: (context,url)=>Container(
        ...
        Flexible(
          child: Container(
            child: TextField(
              ...
              decoration: InputDecoration.collapsed(hintText: "Write Here",hintStyle: TextStyle(color: Colors.grey)
              ),
              focusNode: focusNode,
            ),
  ...
  void onSendMessage(String contentMsg,int type){
    ...
    if(contentMsg!=""){
            ...
            var docRef=Firestore.instance.collection("message").document(chatId).collection(chatId).document(DateTime.now().millisecondsSinceEpoch.toString());
